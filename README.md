...
- Each box shows a `思考翻译 · 块 N · done/total` title (N counts thinking blocks within the same message) plus per-line status: waiting, streaming partial text, final text, or a failure message. Translation updates repaint through the requesting component's own `requestRender`, and the effective config is only ever replaced, never cleared — otherwise the key material for the lookup vanishes and every box on screen blanks out the moment the host starts the next message, which a tool result also is.
- Falls back to a persistent widget above the editor for translations that arrive too late to be painted. The host commits a thinking block's rows to native scrollback once the assistant message reaches its tool call, after which no extension repaint can rewrite them; a translation landing after that would be invisible. The widget content is derived state, not a one-shot guess: it lists the current block's translations that completed without the box ever drawing them, a later repaint that does draw one removes it again, and a translation completing in a newer thinking block drops the older block's rows. So a slow repaint cannot strand a duplicate in the widget, and the widget clears itself as soon as the next block's translations land rather than waiting for the next user turn.
...
1. The extension tracks the message in flight: `message_start` reloads the effective config and resets the per-message block state, `message_update` records each thinking block's raw text (and marks the block closed on `thinking_end`), and `message_end` closes whatever the provider left open. Note that a tool result is its own message, so `message_start` fires mid-turn — which is why the config is replaced there rather than cleared.
...
7. When a translation completes, the extension checks whether the box ever drew that cell. If not, the translation goes into the widget above the editor; if a repaint draws it shortly after, the widget entry is withdrawn, and a completion in a newer thinking block clears the older block's entries.
